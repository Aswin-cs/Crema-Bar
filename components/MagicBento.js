              ...
              >
                {card.src && (
                  <div className="absolute inset-0 z-0 overflow-hidden" style={{ borderRadius: 'inherit' }}>
                    {card.type === 'video' ? (
                      <video src={card.src} autoPlay loop muted playsInline className="w-full h-full object-cover opacity-60" />
                    ) : (
                      <img src={card.src} alt={card.title} className="w-full h-full object-cover opacity-60" />
                    )}
